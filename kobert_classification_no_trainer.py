...
from utils import CLDatasetForClassification, nt_xent, nt_xent_sup, task_to_keys
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The name of the kobest task to train on.",
...
def main():
    ...
    send_example_telemetry(f"run_{args.task_name}_no_trainer", args)
    ...
    if args.task_name is not None:
        ...
        raw_datasets = load_dataset("skt/kobest_v1", args.task_name)
    else:
    ...
    if 'kobert' in args.model_name_or_path:
    ...
    # dataset for contrastive learning
        ...
        train_dataset = CLDatasetForClassification(raw_datasets['train'], tokenizer, args)
    ...
    def collate_fn_batch_encoding_pairwise(batch):
        try:
            ...
            sent1_toks = tokenizer(list(sent1), list(sent2),  max_length=args.max_length, padding=True, truncation=True, add_special_tokens=True, return_tensors="pt")
            sent2_toks = tokenizer(
                ...
                return_tensors="pt")
            # sent1_length = len(sent1_toks.input_ids[-1])
            # sent2_length = len(sent2_toks.input_ids[-1])
            sent1_length = sent1_toks.input_ids.shape[-1]
            sent2_length = sent2_toks.input_ids.shape[-1]
            if sent1_length > sent2_length:
                print(f'New length: {sent1_length}')
                sent2_toks = tokenizer(
                    list(sent1),
                    list(sent2), 
                ...
                print(f'New length: {sent2_length}')
    ...
    train_dataloader = DataLoader(train_dataset, shuffle=True, collate_fn=data_collator_train, batch_size=args.per_device_train_batch_size, drop_last=True)        
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers(f'{args.task_name}_{args.seed}', experiment_config)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.cl_method is None:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if completed_steps % 100 == 0:
                    print(f'{completed_steps=}, loss={loss.item()}')
                ...
                try:
                    if args.cl_method is not None:
                        if args.cl_method == 'scl':
                            pooled1 = model.bert(input_ids=batch1['input_ids'], attention_mask=batch1['attention_mask'], token_type_ids=batch1['token_type_ids']).pooler_output
                            pooled2 = model.bert(input_ids=batch2['input_ids'], attention_mask=batch2['attention_mask'], token_type_ids=batch2['token_type_ids']).pooler_output
                            
                            scl_loss = nt_xent_sup(pooled1, pooled2, batch1['labels'], args.temperature)
                            # scl_loss = nt_xent_sup(pooled1, pooled1, batch1['labels'], args.temperature)
        ...
        if args.with_tracking:
            accelerator.log(
                {
                    "f1" if args.task_name is not None else "glue": eval_metric,
    ...
    remove_suffices = ['bin', 'pkl']
    
    for path in os.scandir(args.output_dir):
        if path.is_dir():
            files = os.scandir(path)
            for filename in files:
                for suffix in remove_suffices:
                    # print(filename.name)
                    if filename.is_file() and filename.name.endswith(suffix):
                        os.remove(filename)
        elif path.is_file():
            for suffix in remove_suffices:
